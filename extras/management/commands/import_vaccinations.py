...
from places.models import Vaccination
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for row in result:
            ...
            row['description'] = row.pop("vaccination_description")
            obj, is_created = Vaccination.objects.update_or_create(
                name=row.pop('vaccination_name'),
                defaults=row
            )
            if is_created:
                total += 1
            obj.save()
        message = 'Imported %i vaccination' % total
